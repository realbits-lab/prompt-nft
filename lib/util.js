...
export function PromptDialog({ inputOpenDialog, inputDecryptedPrompt }) {
  const [openDialog, setOpenDialog] = React.useState(false);

  React.useEffect(() => {
    console.log("call useEffect()");
    console.log("inputOpenDialog: ", inputOpenDialog);
    console.log("inputDecryptedPrompt: ", inputDecryptedPrompt);

    if (
      (typeof inputDecryptedPrompt === "string" ||
        inputDecryptedPrompt instanceof String) &&
      inputDecryptedPrompt.length > 0
    ) {
      setOpenDialog(inputOpenDialog);
    } else {
      setOpenDialog(false);
    }
  }, [inputOpenDialog, inputDecryptedPrompt]);
...
export async function handleCheckPrompt({
  ...
  if (true) {
    ...
    const decodedPrompt = await decryptData({
...
export const writeRentingDataState = atom({
  key: `writeRentingDataState/${v1()}`,
  contents: [],
});

export const readRentingDataState = selector({
  key: `readRentingDataState/${v1()}`,
  get: ({ get }) => {
    const rentingDataState = get(writeRentingDataState);
    return rentingDataState;
  },
});
